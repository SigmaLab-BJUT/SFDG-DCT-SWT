...
class EfficientNet(nn.Module):
    def __init__(self,
        ...
        self.cbam1 = CBAM(down_s1_output_c*2)
        self.down_s1mix = ConvBNActivation(in_planes=down_s1_output_c*2,
                                         out_planes=adjust_channels(32),
                                         kernel_size=3,
                                         stride=2,
                                         norm_layer=norm_layer,
                                         activation_layer=nn.SiLU)
        down_s1mix_output_c = adjust_channels(32)
        self.cbam2 = CBAM(down_s1mix_output_c)
    ...
    def forward(self, x: Tensor, dctx: Tensor) -> Tensor:


        x1 = self.down_s1(x)

        dctx1 = self.dct_s1(dctx)

        xx = self.x_concat(x1,dctx1)
